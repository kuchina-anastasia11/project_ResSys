...
import numpy as np
from utils import name_processing, desc_processing, action_type_processor, key_skills_coverage
...
class predictor:
    ...
    def cos_sim(self, sentence1, sentence2):
        embeddings = self.encoder.encode([sentence1, sentence2],normalize_embeddings=True)
    ...
    def recomend(
        ...
        for vacancy_id in pred_id:
            ...
            vacancy = self.data_dict.loc[vacancy_id]
            n,d = self.get_names_desc_sim(users_vacancy_name, vacancy["name"], users_vacancy_desc, vacancy["description"])
            vacancy_names.append(n)

            vacancy_desc.append(d)

            vacancy_keyskills.append(key_skills_coverage(users_vacancy_keyskills, set(vacancy["keySkills"])))
        ...
        ranker_data['vacancy_salary_from'] = ranker_data['vacancy_salary_from'].fillna((ranker_data['vacancy_salary_from'].mean()))
        ranker_data['vacancy_salary_to'] = ranker_data['vacancy_salary_to'].fillna((ranker_data['vacancy_salary_to'].mean()))
